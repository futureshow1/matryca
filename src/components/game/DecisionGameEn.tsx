...
import { SCENARIOS, scoreToLevel } from '../../data/scenarios-en';
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {SCENARIOS.map((sc) => {
            ...
            return (
              ...
              >
                ...
                >
                  In reality: {sc.reality.substring(0, 140)}…
                </p>
              </div>
            );
          })}
        ...
        >
          ...
          <a href={makeUrl('/en/who-revealed/')} style={btnStyle('ghost')}>
            Meet the 12 whistleblowers →
          </a>
        </div>
        ...
        >
          Your answers are not saved anywhere — the portal collects no data.
        </p>
      </div>
    );
  }
      ...
      {showConsequence && currentChoice && (
        <>
          ...
          <button onClick={next} style={btnStyle('primary')}>
            {currentIdx === SCENARIOS.length - 1 ? 'See your result' : 'Next scenario →'}
          </button>
        </>
      )}
    </div>

---
id: D02
slug: d02-amazon-alexa
group: D
group_label: Amazon, Uber
title: Amazon Alexa
year_revealed: 2019
last_updated: '2026-04-17'
companies:
- Amazon
status: concluded
jurisdictions:
- US
- UK
- DE
- global
legal_basis:
- Stanowe prawa konsumenckie
- ECPA
- class action
fines:
- amount: 25000000
  currency: USD
  authority: FTC
  date: 2023-05
  category: regulatory_fine
  status: paid
- amount: 95000000
  currency: USD
  authority: '*In re Amazon Alexa Privacy*'
  date: 2024-10
  category: class_action_settlement
  status: paid
whistleblowers:
- le-bonniec
- snowden
journalists:
- Alex Hern
related_cases:
- D03
- D01
- C03
- A02
- B03
tags:
- dzieci
- wyborcy
- pracownicy
- coppa
- bipa
- pozew-zbiorowy
categories:
- privacy
geo_coords: []
priority: TOP
card_complete: true
word_count: 2717
explainer:
  src: /animations/d02-amazon-alexa.html
  duration: '60s'
  title: Amazon Alexa — tysiące pracowników słuchało twojej kuchni
subtitle: Ludzkie odsłuchiwanie nagrań z kuchni, sypialni i rozmów prywatnych
years_active:
- 2014
- 2019
---

# D02 — Amazon Alexa: Ludzkie odsłuchiwanie nagrań z kuchni, sypialni i rozmów prywatnych

> **Kategoria:** Smart speakers / always-on listening / human review / AI trenowanie
> **Firma/firmy:** Amazon
> **Lata:** 2014 (debiut Echo) – 2019 (ujawnienie human review) – 2024 (ugoda class action)
> **Status:** Zakończone ugody; praktyka zmieniona na opt-out; nagrania nadal zbierane
> **ID karty:** D02

---

## Metadane

| Pole | Wartość |
|------|---------|
| Kraj/region | Globalnie; ogniska postępowań: USA, Niemcy, UK |
| Rok ujawnienia | 10 kwietnia 2019 (*Bloomberg*, Matt Day, Giles Turner, Natalia Drozdiak) |
| Lata trwania praktyki | 2014–2019 (domyślne human review) |
| Łączna kara | 95 000 000 USD (class action 2024); brak kar regulacyjnych specyficznie za human review |
| Waluta | USD |
| Podstawa prawna | Stanowe prawa konsumenckie (Illinois BIPA, Washington BPA), ECPA, class action |
| Sygnalista/odkrywca | Bloomberg (inside sources); później VICE i The Guardian z kolejnymi ujawnieniami |
| Liczba poszkodowanych | ~100+ mln użytkowników Alexa globalnie |
| Status (na dziś) | Amazon wprowadził opt-out; human review nadal istnieje; class action rozliczony 2024 |

---

## TL;DR

**Amazon Echo** — głośnik z asystentem głosowym **Alexa** — zadebiutował w listopadzie 2014. Do 2019 Amazon sprzedał **>100 mln urządzeń Echo**. **10 kwietnia 2019** *Bloomberg* (Matt Day, Giles Turner, Natalia Drozdiak) opublikował śledztwo: **Amazon zatrudniał tysiące pracowników i kontraktorów na całym świecie**, którzy **odsłuchiwali prywatne nagrania Alexa** z domów klientów. Zespoły operowały w **Boston, Costa Rica, Indiach, Rumunii**. Każdy pracownik słuchał **ok. 1 000 klipów dziennie**. Zadaniem było **poprawianie transkrypcji** do trenowania AI.

Nagrania, które odsłuchiwali:
- **Rozmowy prywatne** domowników (Alexa aktywowała się przypadkowo przy słowach podobnych do "Alexa" — "election", "Alexis", nazwy ulic w Teksasie)
- **Śpiewająca pod prysznicem** (klipy dzielone jako anegdoty wewnętrzne)
- **Dzieci mówiące z rodzicami**
- **Prawdopodobne napaści seksualne** (pracownicy zgłosili — Amazon odpowiedział: "nie naszym zadaniem interwencja")
- **Dyskusje o przemocy domowej**

Amazon nie informował użytkowników, że **człowiek może usłyszeć nagrania**. Domyślnie wszystkie nagrania były **używane do trenowania AI**. Bloomberg:
> *"The team includes a mix of contractors and full-time Amazon employees who work in outposts from Boston to Costa Rica, India, and Romania. They transcribe, annotate and then feed the data back into the system."*

Po publikacji Amazon w **3 dni** wprowadził **opt-out**. Domyślnie nadal było **włączone**. Meta, Apple, Google (które robiły podobnie) szybko ogłosiły zmiany polityki. Apple **zawiesił human review Siri** na kilka miesięcy.

**31 maja 2023** — FTC nałożyła na Amazon **25 mln USD kary** za naruszenia **COPPA** w związku z Alexa Kids → osobna sprawa, karta D03.

**2022–2024** — class action ***In re Amazon Alexa Privacy Litigation*** w W.D. Washington. Powód wiodący **Jessica Garcia**, reprezentowana przez **Keller Rohrback L.L.P.**, reprezentował ~100 mln użytkowników Alexa. **23 października 2024** — ugoda na **95 000 000 USD**. Illinois residents — szczególne roszczenia pod **BIPA** (Biometric Information Privacy Act).

Sprawa D02 pokazuje, że **głosowe AI assistants były trenowane na prywatnych rozmowach** bez ujawnionej zgody — klasyczny wzorzec "użytkownik nie wie, że człowiek go słyszy". Praktyka dziś jest **opt-out** domyślnie, ale **nagrania głosowe nadal są zbierane** i analizowane (w większości automatycznie, ale część nadal przechodzi przez human review).

---

## Oś czasu

- **Listopad 2014** — Amazon Echo debiut (początkowo tylko dla Prime members).
- **2015–2019** — ekspansja Alexa do setek milionów urządzeń. Echo, Echo Dot, Echo Show, Echo Auto.
- **2014–2019** — Amazon buduje **zespoły human review** w Boston, Costa Rice, Indiach, Rumunii. Pracownicy odsłuchują nagrania do trenowania AI.
- **10 kwietnia 2019** — ***Bloomberg*** publikuje: "Amazon Workers Are Listening to What You Tell Alexa".
- **Kwiecień 2019** — *Guardian*, *VICE Motherboard* — dodatkowe ujawnienia podobnych praktyk w **Apple Siri**, **Google Assistant**, **Facebook Messenger voice clips**.
- **Lipiec 2019** — **Apple wstrzymuje** human review Siri globalnie. Google — częściowe wstrzymanie dla EU. Facebook — wstrzymuje.
- **13 kwietnia 2019** — Amazon wprowadza opt-out dla **human review** (ale domyślnie włączone).
- **Sierpień 2019** — **hamburski DPA Johannes Caspar** (ten sam od Google WiFi Street View) wszczyna postępowanie dot. Alexa w Niemczech.
- **2020–2022** — szereg klas akcji w USA.
- **31 maja 2023** — **FTC kara 25 mln USD** za naruszenia COPPA (→ D03).
- **23 października 2024** — **ugoda 95 mln USD** w *In re Amazon Alexa Privacy Litigation*.
- **2025** — Amazon aktualizuje opcje domyślne: **"Don't save voice recordings"** jako nowa domyślna opcja dla nowych użytkowników.

---

## Mechanizm

### Jak Alexa zbiera nagrania

**1. Always-on mic.**
Echo ma mikrofon aktywny **24/7**. Nagrywa lokalnie w **rolling buffer**. Gdy wykryje **wake word** ("Alexa", "Amazon", "Echo", "Computer"), zaczyna **wysyłanie do chmury Amazon**.

**2. Problem z false wake.**
Słowa podobne do "Alexa" — **"election"**, **"Alexis"**, **"Alex"**, nazwy ulic jak **"Alexander"** — mogą aktywować urządzenie. Bloomberg ujawnił, że w Texas pracownicy odsłuchali **wiele nagrań rozmów politycznych**, gdy ludzie mówili "election".

**3. Wysyłanie do chmury.**
Po detekcji wake word 5-10 sekund audio przed i po → wysyłane do serwerów Amazon. Tam:
- **Automatyczna transkrypcja** (Alexa AI)
- **Rozpoznanie intencji** (NLP)
- **Odpowiedź głosowa**
- **Zapisanie nagrania** z timestamp, user ID, lokalizacją

**4. Human review.**
**~0,1%** nagrań szło do human review. Ale 0,1% × **miliardy nagrań** × **100+ mln użytkowników** = **miliony godzin audio** przetwarzanych przez ludzi. Zadania pracowników:
- Sprawdzić, czy transkrypcja jest poprawna
- Oznaczyć gatunek mówiącego (mężczyzna/kobieta/dziecko)
- Oznaczyć emocje
- Oznaczyć język / akcent
- Poprawić transkrypcję, jeśli była błędna

**5. Karmienie AI.**
Transkrypcje + audio + adnotacje **wracają do modelu**. Model uczy się lepiej rozpoznawać akcenty, gatunki, intencje. Im więcej human review, tym lepszy model.

### Kluczowe ujawnienia Bloomberg

**Pracownicy dzielili się anegdotami w Slacku Amazon:**
- Kobiety śpiewające pod prysznicem (**"it's funny"** — cytat)
- Dzieci przeklinające
- Ludzie kłócący się

**Pracownicy słyszeli potencjalne przestępstwa:**
- **Co najmniej dwa przypadki** prawdopodobnej **napaści seksualnej** (według źródeł Bloomberg).
- Pracownicy zgłaszali przełożonym. Odpowiedź: **"nie naszym zadaniem interwencja; kontynuujcie pracę"**.

**Lokalizacje pracowników:**
- **Boston** (USA) — pełnoetatowi Amazon
- **Costa Rica** — kontraktorzy
- **Rumunia** (Bukareszt) — kontraktorzy
- **Indie** (Chennai) — kontraktorzy
- **Polska, Hiszpania, Niemcy** — zidentyfikowane później

### Dlaczego to było naruszenie

1. **Brak ujawnienia** — Amazon w 2019 **nie mówił** użytkownikom, że ludzie słuchają.
2. **Domyślnie włączone** — nie było opt-out do kwietnia 2019.
3. **Nagrania dzieci** — COPPA wymaga zgody rodzica (→ D03).
4. **Nagrania przypadkowe** — false wake activations → pracownicy słuchali intymnych rozmów bez jakiejkolwiek zgody.
5. **Naruszenia ECPA** — Electronic Communications Privacy Act zakazuje przechwytywania komunikacji bez zgody (klasyczne **wiretapping**).
6. **BIPA** (Illinois) — analiza głosu = informacja biometryczna. Zbieranie bez zgody = naruszenie.

---

## Odkrycie

### Bloomberg śledztwo

***Matt Day, Giles Turner, Natalia Drozdiak*** — trzech dziennikarzy Bloomberg. Bazę: **7 źródeł** wewnątrz Amazon i firm kontraktowych. Klucz: uzyskanie zarzutów, że pracownicy mieli zakaz opisywania praktyk osobom spoza firmy.

Publikacja: **10 kwietnia 2019**: "Amazon Workers Are Listening to What You Tell Alexa". Efekt: **natychmiastowy globalny skandal**.

### Guardian → Apple Siri

***Alex Hern*** (*The Guardian*) w **26 lipca 2019** — podobna historia dla Apple Siri. Anonimowy whistleblower Apple kontraktor (później zidentyfikowany jako **Thomas le Bonniec**, Francja) opisał, że Apple pracownicy odsłuchiwali Siri nagrania, w tym:
- **Rozmowy z lekarzem** (dyktafonowe nagrania)
- **Intymne rozmowy** (mikrofon aktywowany przez dźwięki zamka)
- **Dealerów narkotyków** omawiających tranzakcje

Le Bonniec zgłosił do francuskiego CNIL w 2020.

### VICE/Motherboard → Facebook

***Sam Biddle, Joseph Cox*** (*Motherboard*) — 13 sierpnia 2019 — Facebook transkrybował nagrania **Messenger voice chats** przez kontraktorów.

### Pierwsze publikacje

- **10 kwietnia 2019** — Matt Day, Giles Turner, Natalia Drozdiak, *Bloomberg*
- **26 lipca 2019** — Alex Hern, *The Guardian*, Apple Siri
- **13 sierpnia 2019** — *Motherboard*, Facebook Messenger

---

## Osoby kluczowe

### Amazon

- **Jeff Bezos** — CEO Amazon do 2021.
- **Andy Jassy** — CEO AWS, od 2021 CEO Amazon.
- **Rohit Prasad** — VP Alexa AI, główny inżynier.
- **Dave Limp** — SVP Devices and Services (Echo), do 2023.

### Dziennikarze

- **Matt Day** (*Bloomberg*) — główny odkrywca.
- **Alex Hern** (*Guardian*) — Apple Siri.
- **Joseph Cox, Sam Biddle** (*Motherboard/Vice*) — Facebook i szersza analiza.

### Sygnaliści

- Anonimowi pracownicy Amazon (7 osób dla Bloomberg).
- **Thomas le Bonniec** — Apple Siri kontraktor we Francji.

### Regulatorzy

- **Johannes Caspar** (Hamburg DPA) — otworzył postępowanie przeciw Amazon w sierpniu 2019.
- **CNIL** (Francja) — postępowanie po le Bonniec.

### Klasy

- **Jessica Garcia** — powód wiodący w *In re Amazon Alexa Privacy Litigation*.
- **Keller Rohrback L.L.P.** — kancelaria klasy.

---

## Reakcja firmy

### Amazon

**Etap 1: obrona (10 kwietnia 2019).** Oświadczenie: *"We take the security and privacy of our customers' personal information seriously. We only annotate an extremely small sample of Alexa voice recordings in order to improve the customer experience."*

**Etap 2: szybkie wprowadzenie opt-out (13 kwietnia 2019).** 3 dni po publikacji Bloomberg, Amazon wprowadza opcję:
- Ustawienia → Alexa Privacy → **Manage Your Alexa Data** → **Help Improve Alexa** → wyłącz

**Etap 3: komunikat publiczny o praktyce.** Rohit Prasad (VP Alexa AI) pisze blog post wyjaśniający, jak działa human review i dlaczego jest "niezbędne".

**Etap 4: reforma defaults (2019–2020).**
- Opcja **"Don't save voice recordings"** (ale nowa domyślnie = zapisywanie)
- Opcja **"Delete voice recordings after 3 months"** (auto-delete)
- Opcja **"Delete what I just said"** (po komendzie)

**Etap 5: klasowe ugody (2022–2024).**
- Illinois BIPA class actions — osobne ugody
- Główna: 95 mln USD ugoda 2024

---

## Postępowanie prawne

### Jurysdykcje

- **USA federalne** — FTC (25 mln za COPPA, → D03)
- **USA stanowe** — Illinois (BIPA), Washington (BPA), Kalifornia
- **Niemcy** — Hamburg DPA, śledztwo 2019+
- **Francja** — CNIL (po sprawie Siri le Bonniec)

### Podstawa prawna

- **ECPA** — Electronic Communications Privacy Act
- **Illinois BIPA** (Biometric Information Privacy Act) — głos jako biometria
- **Washington BPA** — Biometric Privacy Act
- **California CCPA/CPRA**
- **RODO art. 6, 9** (w UE) — głos może być danymi wrażliwymi

### Kluczowe etapy

| Data | Etap |
|------|------|
| 10 kwietnia 2019 | Bloomberg publikacja |
| 13 kwietnia 2019 | Opt-out wprowadzony |
| 31 maja 2023 | FTC kara COPPA 25 mln |
| 23 października 2024 | Ugoda klasy 95 mln |

---

## Kary i ugody

| Data | Organ | Kwota | Jurysdykcja | Podstawa |
|------|-------|-------|-------------|----------|
| 31 maja 2023 | FTC | 25 000 000 USD | USA (COPPA) | → D03 |
| 23 października 2024 | *In re Amazon Alexa Privacy* | 95 000 000 USD | USA klasa | BIPA, ECPA, state laws |

**Łącznie: ~120 mln USD** (z COPPA osobną karą).

---

## Precedensy i implikacje

### Dla prawa USA

- **BIPA jako potężne narzędzie** — głos = biometria. 500 USD per naruszenie × miliony = potencjalnie miliardy.
- **Wzmocnienie FTC** dla smart speakers.

### Dla prawa UE

- **Hamburg DPA Caspar** — śledztwa Alexa, Siri 2019+. Nie zakończyły się karami, ale wzmocniły **EDPB Guidelines on Voice Assistants** (2021).

### Dla praktyki Big Tech

- **Apple** — wstrzymał human review Siri; później wprowadził **opt-in domyślny**.
- **Google Assistant** — analogiczne zmiany.
- **Meta (Facebook Portal, WhatsApp voice)** — wycofał human review.
- **Microsoft Cortana** — wycofał większość funkcji voice 2020.

---

## Pozwy zbiorowe

| Sprawa | Sąd | Stan | Wartość | Poszkodowani |
|--------|-----|------|---------|--------------|
| *In re Amazon Alexa Privacy Litigation* | W.D. Wash. | Ugoda 10.2024 | 95 000 000 USD | ~100 mln użytkowników |

---

## Wnioski dla obywateli

### Co to dla mnie znaczy?

Jeśli masz Echo, Apple HomePod, Google Nest — **pamiętaj, że mikrofon jest aktywny 24/7**. Nawet jeśli "wake word" działa dobrze, **false activations** zdarzają się codziennie. Fragmenty twoich rozmów prywatnych **już są** w bazach Amazon/Apple/Google. Historia Alexa pokazuje, że **ludzie mogli je odsłuchać** — i prawdopodobnie odsłuchiwali.

### Jak chronić się?

1. **Wyłącz "Help Improve Alexa"**: Amazon aplikacja → Settings → Alexa Privacy → Manage Your Alexa Data → **Off**.
2. **Włącz auto-delete**: Settings → Alexa Privacy → Manage Your Alexa Data → Delete after 3 months.
3. **Wyłącz mikrofon fizycznie** gdy nie używasz — większość Echo ma przycisk.
4. **Rozważ "Stop Listening"** przed rozmowami wrażliwymi: powiedz "Alexa, stop listening".
5. **Nie umieszczaj Echo w sypialni/łazience/pokoju dziecka**.
6. **Rozważ Home Assistant + Rhasspy** — lokalny voice assistant, brak chmury.

### Dla rodziców

7. **Echo Dot Kids** — ma szczególne ograniczenia, ale nadal zbiera dane. Rozważ alternatywy.
8. **Alexa Kids skills** — były głównym obszarem naruszeń COPPA (→ D03).

### Jakie mam prawa?

**W USA:**
- **BIPA Illinois** — jeśli jesteś rezydentem, możesz indywidualnie pozwać za zbieranie głosu bez zgody.
- **ECPA** — zakaz intercepcji komunikacji.

**W UE (RODO):**
- **Art. 9** — głos może być daną wrażliwą (zdrowotną, biometryczną).
- **Art. 17** — prawo do usunięcia nagrań.
- **Art. 82** — odszkodowanie za szkodę.

### Uwaga dla mediatorów, prawników, ofiar przemocy domowej

- **Tajemnica zawodowa** — **NIE prowadź konsultacji prawnych/mediacyjnych w pomieszczeniu z Echo/HomePod/Google Nest**. Nawet jeśli wake word nie zostanie wywołany, false activations mogą zarejestrować fragmenty.
- **Dla ofiar przemocy** — były partner, który znał hasło Amazon, może słuchać historii nagrań Alexa. Zmień hasło, wyloguj jego urządzenia.
- **Dla prawników** — wyłącz Echo w kancelarii. Apple HomePod ma lepszą reputację prywatności (E2E), ale nadal mikrofon jest 24/7.

---

## Ciekawostki

- **"Śpiewająca pod prysznicem"** — jeden z anegdotycznych cytatów Bloomberg — pracownicy Amazon w Bostonie dzielili się w Slacku anegdotami z najlepszymi klipami. **"It's funny"** — cytat. Krytycy: to jest **kultura niepoważnego podejścia do prywatności** klientów.
- **Pracownik odsłuchujący napaść seksualną** — najbardziej wstrząsający detal Bloomberg. Pracownicy zgłosili potencjalne przestępstwo; Amazon odpowiedział: **"nie naszym zadaniem interwencja"**. To wywołało debatę: czy firma ma **obowiązek zgłaszać** organom ścigania przestępstwa usłyszane w nagraniach?
- **Rumunia, Bukareszt** — główne europejskie hub Amazon do human review. Pracownicy mówili po angielsku, hiszpańsku, francusku, niemiecku. W 2022 Amazon przeniósł wiele operacji do Indii.
- **Thomas le Bonniec** — francuski kontraktor Apple Siri. Zgłosił do CNIL w 2020. Jego whistleblowing był w stylu *Snowden light* — nie ujawnił danych, tylko praktykę. Apple reakcja: zatrudnił własnych pracowników zamiast outsourcingu (pozytywne), ale nie wypłacił odszkodowania.
- **Apple reakcja była najlepsza** — wstrzymał human review globally w lipcu 2019, potem wprowadził **opt-in domyślny**. Tim Cook oświadczenie: *"We will not improve Siri at the cost of our customers' privacy."*
- **"Ring + Alexa paradox"** — Amazon w ten sam dzień (31 maja 2023) otrzymał dwie kary FTC: Ring (5,8 mln) + Alexa COPPA (25 mln) = **30,8 mln łącznie**. Rekordowy dzień złych nowości dla smart home Amazon.
- **Polski rynek** — Echo jest w Polsce od 2019, ale rynek jest mały w porównaniu z USA/UK/DE. Alexa po polsku została wprowadzona dopiero 2021. Lokalizacja nagrań z polskich Echo — niejasna (prawdopodobnie w chmurze AWS w EU, ale human review mogło być w Costa Rica/India).
- **Hamburski Caspar znowu** — Johannes Caspar (sprawa Google WiFi Street View — B01) otworzył postępowanie przeciw Amazon w sierpniu 2019. Amazon zobowiązał się do zmian w UE wcześniej niż w USA. Klasyczny **Brussels effect**.
- **False wake "election"** — słowo "election" brzmi podobnie do "Alexa" z texańskim akcentem. Bloomberg ujawnił, że pracownicy Amazon odsłuchali **tysiące nagrań z rozmów politycznych** podczas kampanii 2016 i 2018 w USA. Ludzie rozmawiający o wyborach przy Echo — ich komentarze były odsłuchiwane przez obcych pracowników w Indiach, Rumunii.
- **"It knows your address"** — krytycy Amazon zauważali, że human reviewers widzieli **user ID, typ urządzenia, lokalizację IP** (miasto). W teorii możliwe było **powiązanie głosu z konkretną osobą**.
- **Amazon Alexa+ (2025)** — nowa wersja Alexa z **generatywnym AI**. Amazon obiecuje **lepszą prywatność**, ale skeptycy (włącznie z Edward Snowden, Bruce Schneier) ostrzegają, że LLM-powered voice assistants **mają jeszcze większą** powierzchnię ataku.
- **Spotify Car Thing** — nieoficjalne smart speaker Spotify (2022, później wycofany) miał **lokalne** przetwarzanie bez chmury. Pokazał, że **inny model jest możliwy**.
- **"Apparently"** — jedno z często wypowiadanych słów, które przez długi czas **aktywowały Alexa** (blisko fonetycznie do "Alexa" w niektórych akcentach). Amazon poprawił w 2021, ale lista false wakes nadal istnieje.

---

## Źródła

1. Matt Day, Giles Turner, Natalia Drozdiak, "Amazon Workers Are Listening to What You Tell Alexa", *Bloomberg*, 10 kwietnia 2019. URL: https://www.bloomberg.com/news/articles/2019-04-10/is-anyone-listening-to-you-on-alexa-a-global-team-reviews-audio (dostęp: 2026-04-17)

2. Alex Hern, "Apple contractors 'regularly hear confidential details' on Siri recordings", *The Guardian*, 26 lipca 2019.

3. Federal Trade Commission, "FTC and DOJ Charge Amazon with Violating Children's Privacy Law by Keeping Kids' Alexa Voice Recordings Forever and Undermining Parents' Deletion Requests", 31 maja 2023.

4. *In re Amazon Alexa Privacy Litigation*, W.D. Wash., Case No. 2:19-cv-00910.

5. Amazon blog post, Rohit Prasad, "Alexa and Privacy", kwiecień 2019.

6. Thomas le Bonniec, zeznanie do CNIL, 2020.

7. Hamburg DPA (Johannes Caspar), decyzja ws. Alexa, sierpień 2019.

8. European Data Protection Board, "Guidelines 02/2021 on Virtual Voice Assistants", 2021.

9. Illinois Biometric Information Privacy Act (740 ILCS 14).

10. Keller Rohrback L.L.P., komunikaty prasowe o ugodzie, październik 2024.

11. Joseph Cox, Sam Biddle, "Facebook Contractors Transcribe Audio from Messenger", *Motherboard/Vice*, 13 sierpnia 2019.

12. Tim Cook, oświadczenie o Siri, 2019.

13. Bruce Schneier, "Alexa and the Privacy Paradox", blog, 2019.

14. CNBC, NPR, CNN Business — szeroka koordynowana relacja z 31 maja 2023.

15. Natalia Drozdiak, follow-up artykuły w *Bloomberg* 2019–2020.

---

*Ostatnia aktualizacja: 2026-04-17*
*Karta w bazie: D02_amazon_alexa.md*
